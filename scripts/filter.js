...
let filter = {
  ...
  orders: function () {
    ...
    document.querySelectorAll("#orders .col").forEach((elem) => {
      ...
      filter.storage.push({
        ...
        count: b[2].innerHTML,
        reason: (reason = elem.querySelector("p.reason"))
          ? reason.innerHTML
          : null,
  ...
  filter: function (param, type) {
    ...
    if (param == "all") array = JSON.parse(JSON.stringify(filter.storage));
  ...
  out: function (array, type) {
    ...
    if (type == "products") {
      array.forEach((product) => {
        ...
        data +=
          role != "guest"
            ? `<p class="text-right"><a href="controllers/add_cart.php?id=${product.product_id}" class="text-small">В корзину</a></p>`
            : "";
        data += `</div>`;
      });
      // Запись заказов
    ...
    if (!data) data = "<h3 class='text-center'>Данные отсутствуют</h3>";
